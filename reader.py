...
def calibrate_gauge(gauge_number, file_type):
    ...
    print('opening file {0}.{1}'.format(gauge_number, file_type))
    img = cv2.imread('%s.%s' %(gauge_number, file_type))
    ...
    circles = cv2.HoughCircles(gray, cv2.cv.CV_HOUGH_GRADIENT, 1, 20, np.array([]), 100, 50, int(height*0.35), int(height*0.48))
    # average found circles, found it to be more accurate than trying to tune HoughCircles parameters to get just the right one
    a, b, c = circles.shape
    x,y,r = avg_circles(circles, b)

    #draw center and circle
    cv2.circle(img, (x, y), r, (0, 0, 255), 3, cv2.CV_AA)  # draw circle
    cv2.circle(img, (x, y), 2, (0, 255, 0), 3, cv2.CV_AA)  # draw center of circle
    ...
    for i in range(0,interval):
        ...
        cv2.putText(img, '%s' %(int(i*separation)), (int(p_text[i][0]), int(p_text[i][1])), cv2.FONT_HERSHEY_SIMPLEX, 0.3,(0,0,0),1,cv2.CV_AA)

    # cv2.imwrite('%s-calibration.%s' % (gauge_number, file_type), img)
...
def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, gauge_number, file_type):
    ...
    # cv2.imwrite('%s-tempdst2.%s' % (gauge_number, file_type), dst2)
    ...
    #cv2.imwrite('%s-lines-2.%s' % (gauge_number, file_type), img)
